...
beforeEach(async () => {
  db = createDb(":memory:");
  migrate(db);
  const projectId = (await insertProject(db, { name: "test", apiKeyHash: "h" })).id;
  actionId = (await insertAction(db, {
    projectId,
    type: "payment",
...
describe("mintApprovalToken", () => {
  it("never stores the plaintext token", async () => {
    ...
    expect(JSON.stringify(await listApprovals(db))).not.toContain(token);
  });

  it("stores a sha256 hex digest instead", async () => {
    await mintApprovalToken(db, actionId);
    const [row] = await listApprovals(db);
...
describe("verifyApprovalToken", () => {
  ...
  it("mutates nothing — not even for an expired token", async () => {
    const { token } = await mintApprovalToken(db, actionId, -1);
    const before = JSON.stringify(await listApprovals(db));

    verifyApprovalToken(db, token).catch(() => {});

    // Expiring the *action* is the route's job, so that a GET stays inert.
    expect(JSON.stringify(await listApprovals(db))).toBe(before);
  });

  it("carries the action id on an expired error so the caller can finalise it", async () => {
    const { token } = await mintApprovalToken(db, actionId, -1);

    verifyApprovalToken(db, token).catch((err: ApprovalTokenError) => {
  ...
  it("reveals nothing about an unknown token", async () => {
    verifyApprovalToken(db, "abc").catch((err: ApprovalTokenError) => {
